...
RenderTexture::RenderTexture(ID3D11Device* device, int textureWidth, int textureHeight, float screenNear, float screenFar)
{
	...
	D3D11_UNORDERED_ACCESS_VIEW_DESC unorderedAccesViewDesc;
	...
	ZeroMemory(&unorderedAccesViewDesc, sizeof(unorderedAccesViewDesc));

	unorderedAccesViewDesc.Format = DXGI_FORMAT_R8G8B8A8_UNORM;
	unorderedAccesViewDesc.ViewDimension = D3D11_UAV_DIMENSION_TEXTURE2D;
	unorderedAccesViewDesc.Texture2D.MipSlice = 0;

	result = device->CreateUnorderedAccessView(m_renderTargetTexture, &unorderedAccesViewDesc, &m_unorderedAccessView);
	...
	m_projectionMatrix = XMMatrixPerspectiveFovLH(((float)XM_PI / 4.0f), ((float)textureWidth / (float)textureHeight), screenNear, screenFar);
